...
if ! command -v python3 >/dev/null; then sk "python3 が無い"; else
  ...
  if printf '%s' "$out" | grep -q "✓ 判定"; then
    ...
    # 🔴 **門の印を列挙しない**（2026-09-06 実測）。ここは `[①②③⚠]` と書いてあった ——
    #   門を足すたびに *門の外に在るこの一行* も直さないと、新しい門が黙って落ちる。
    #   ⚠️ しかも **手元では落ちなかった**: bracket 式は C locale で **バイト**の集合に潰れ、
    #     ①②③⚠ の先頭バイト `E2` が ④⑤● まで拾う。CI(C.UTF-8) では真の文字クラスになり、
    #     公開の報告からだけ ④ 錨 / ⑤ 錨の質 / ● 判定 が消えていた。
    #   ◆ 手元で見えるものが公開でも見えるとは限らない。**列挙をやめれば、両方で同じになる。**
    printf '%s\n' "$out" | grep -E '^  [^ ]' | sed 's/^/     /'
  else
    ng "文書と門番がずれている"
    printf '%s\n' "$out" | grep -E '^  ✗' | sed 's/^/     /'
